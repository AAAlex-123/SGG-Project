...
private:
	...
	template<class T>
	void deleteList(list<T*>* ls);

public:
	list<Entity*>* enemyLs = new list<Entity*>;
	list<Entity*>* playerLs = new list<Entity*>; //uniform access to either 1 or 2 players and definitely no more
...
template <class T>
void GameData::draw(list<T*>* ls) const {
	for (Drawing* dr : ls) {
		dr->draw();
	}
}
